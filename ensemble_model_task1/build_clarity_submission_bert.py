"""
Fine-tune DeBERTa for CLARITY (Task 1) and export ensemble artifacts.
...
Outputs (saved in the current directory):
  ...
  - (Optional) prediction_bert (308 labels) for inspection

Notes:
  - Uses the same train/dev split as the SVM script via clarity_split_indices.npz.
  - Label mapping: Ambivalent->0, Clear Non-Reply->1, Clear Reply->2
  - Run: python ensemble_model_task1/build_clarity_submission_bert.py
"""
...
class Config:
    ...
    num_train_epochs: int = 1  # keep light so it actually finishes
    per_device_train_batch_size: int = 8
    per_device_eval_batch_size: int = 8
    warmup_ratio: float = 0.06
    weight_decay: float = 0.01
    output_dir: str = "outputs/bert_clarity_ensemble"
    seed: int = 42
...
def main():
    ...
    with open(BASE_DIR / "prediction_bert", "w", encoding="utf-8") as f:
        for lbl in test_preds:
            f.write(lbl + "\n")

    print("Saved BERT ensemble files in", BASE_DIR)
    print("  bert_dev_proba.npy")
    print("  bert_test_proba.npy")
    print("  bert_test_preds.npy")
    print("Wrote prediction_bert (308 lines)")
